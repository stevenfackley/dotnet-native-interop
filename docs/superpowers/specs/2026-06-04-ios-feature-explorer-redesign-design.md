...
`OnDeviceLlm` demonstrates a NativeAOT .NET library loaded in-process by native UIs over several
...
### 2. Engine changes (`OnDeviceLlm.Engine`)
...
### 3. FFI ABI changes (`OnDeviceLlm.NativeBridge`, `abi/ondevicellm.h`)

New exports (additive; existing streaming exports untouched):

```c
const char* ondevicellm_features_json(void);          /* JSON array of descriptors; caller frees */
const char* ondevicellm_feature_run(const char* id);  /* JSON {id,result,elapsedMs,ok}; caller frees */
void        ondevicellm_string_free(const char* s);   /* frees strings returned above */
```
...
- Strings are heap UTF-8 via `NativeText.Allocate`; the Swift caller copies with `String(cString:)`
  then calls `ondevicellm_string_free`.
- JSON via a **source-generated `JsonSerializerContext`** for `FeatureDescriptor` / `FeatureRun`
  (AOT-safe; no reflection).
- `abi/ondevicellm.h` updated to declare the three new exports under a "Pattern 3 — structured"
  section.
...
ios/
  ...
  Apps/
    ...
    HTTP/                 (phase 2)
    SQLite/               (phase 3)
  Frameworks/             ondevicellm.xcframework (built on Mac)
  project.yml             one project, target OnDeviceLlmFFI = Shared + Apps/FFI
```

- `OnDeviceLlmFFI` target: bundle id `com.ondevicellm.ffi`, display name "C# Features · FFI".
- `FFIFeatureService` calls the three new FFI exports on a background `Task` and decodes JSON.
- The existing `ios/OnDeviceLlmApp/` is migrated into this structure (its working FFI bits become
...
- **Add** the new `ios/Shared/` and `ios/Apps/FFI/` files; repoint `project.yml` to the
  `OnDeviceLlmFFI` target.
- **Superseded** by the new shell: the old single-target views/VMs
  (`OnDeviceLlmApp/Views/{ContentView,TokenStreamView,MetricsRow,PatternInfoPanel}.swift`,
...
- Bundle id moves to `com.ondevicellm.ffi` → installs alongside the current app (distinct apps per
  transport is intended).

## Data flow

`FeaturesView.task` → `viewModel.load()` → `FFIFeatureService.descriptors()` →
`ondevicellm_features_json()` (background) → decode → `descriptors` (UI groups by version).
Tap Run → `viewModel.run(id)` → `ondevicellm_feature_run(id)` (background) → decode →
...
- **Engine (Windows, no device):** throwaway console project in `%TEMP%` referencing
  `OnDeviceLlm.Engine.csproj` — call `Descriptors` and `Run(id)`, assert non-empty + `Ok` true + a
  plausible `elapsedMs`. Delete after. (No probes committed to the repo.)
- **Managed build:** `dotnet build OnDeviceLlm.slnx -c Release` → 0 errors.
- **iOS (Mac mini):** the FFI ABI changed, so the framework **must** be rebuilt:
  `bash build/build-ios-framework.sh`, then build/sign/install `OnDeviceLlmFFI` to the iPad and
...
- **HTTP app:** `HTTPFeatureService` over the raw-socket server; add `GET /features` (descriptor JSON)
  and `GET /feature/{id}/run` (result JSON) to `HttpRaw/`. New target `OnDeviceLlmHTTP`.
