...
/// Both symbols match the declarations in <c>abi/dni.h</c> exactly.
...
internal static unsafe class GrpcExports
{
    ...
    /// </param>
    /// <returns>
    /// <c>DNI_OK (0)</c> on success;
    /// <c>DNI_INVALID_ARGUMENT (-2)</c> for a null or empty path;
    /// <c>DNI_INTERNAL (-5)</c> on any unexpected failure.
    /// </returns>
    [UnmanagedCallersOnly(EntryPoint = "dni_grpc_start")]
    ...
    /// </summary>
    /// <returns>
    /// <c>DNI_OK (0)</c> on success or if the server was not running;
    /// <c>DNI_INTERNAL (-5)</c> on any unexpected failure.
    /// </returns>
    [UnmanagedCallersOnly(EntryPoint = "dni_grpc_stop")]
