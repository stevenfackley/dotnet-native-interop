...
# FAST device-only variant of build-ios-framework.sh: publishes OnDeviceLlm.NativeBridge for the
...
[ -f "${DEVICE_PUBLISH}/ondevicellm.dylib" ] || { echo "ERROR: ondevicellm.dylib not found"; exit 1; }

echo "[2/3] Fixing dylib install name..."
install_name_tool -id "@rpath/${FRAMEWORK_NAME}.framework/${FRAMEWORK_NAME}" "${DEVICE_PUBLISH}/ondevicellm.dylib"
...
<dict>
  ...
  <key>CFBundleExecutable</key><string>ondevicellm</string>
  <key>CFBundleIdentifier</key><string>com.ondevicellm</string>
  <key>CFBundleInfoDictionaryVersion</key><string>6.0</string>
  <key>CFBundleName</key><string>OnDeviceLlm</string>
