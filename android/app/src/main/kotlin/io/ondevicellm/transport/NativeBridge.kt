...
/**
 * Kotlin facade for the JNI shim (ondevicellm_jni.so).
 *
 * Load order matters:
 *   1. "ondevicellm"     — the NativeAOT library; all C symbols must be resident first.
 *   2. "ondevicellm_jni" — this shim that calls those symbols.
 *
 * Both loads are performed in [OnDeviceLlmApp] (or MainActivity) so that any
...
public object NativeBridge {
    ...
    /** Calls ondevicellm_initialize(). Returns 0 on success, negative NativeStatus. */
    public external fun nativeInitialize(): Int

    /** Calls ondevicellm_shutdown(). */
